...
The World Health Organization recomends PM2.5 levels below $35ug/m^3$ for a daily average and below $12ug/m^3$ for a yearly average. The daily and yearly recomended levels are ploted in red and blue respectively. Only durring the summer months are the PM2.5 levels below the WHO recomended daily limit.
...
df$BCPM25 <- BoxCox(df$PM2.5, 0.2931797) %>%
    difference(24)
...
Differencing PM2.5 centers the data, and using a log transform centers the data, giving a stationary time series.
...
```
The BoxCox transformation restuls in stable variances.
